...
export interface AgreementProvider {
     ...
     */
    sendAgreement(agreementId: string, contactEmail: string): Promise<{ success: boolean; externalRef?: string }>;

    /**
     * Checks the current status of an agreement from the provider's perspective.
     */
    getAgreementStatus(agreementId: string): Promise<{ status: AgreementStatus; externalRef?: string }>;

    /**
     * Manually records an acceptance if the provider is the manual provider.
     */
    recordAcceptance(agreementId: string, actorId: string, evidence?: string): Promise<{ success: boolean }>;

    /**
     * Voids an existing agreement.
     */
    voidAgreement(agreementId: string): Promise<{ success: boolean }>;
}
