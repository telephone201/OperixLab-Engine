...
-- ============================================================
-- Reconciliation: legacy requirements table
-- ============================================================
...
ALTER TABLE requirements
    ADD COLUMN IF NOT EXISTS source_pain_id UUID;
...
ALTER TABLE requirements
    ADD COLUMN IF NOT EXISTS priority VARCHAR(50) DEFAULT 'UNKNOWN';

ALTER TABLE requirements
    ADD COLUMN IF NOT EXISTS certainty VARCHAR(50) DEFAULT 'UNKNOWN';

ALTER TABLE requirements
    ADD COLUMN IF NOT EXISTS confidence DOUBLE PRECISION DEFAULT 0.0;

ALTER TABLE requirements
    ADD COLUMN IF NOT EXISTS status VARCHAR(50) DEFAULT 'CANDIDATE';

ALTER TABLE requirements
    ADD COLUMN IF NOT EXISTS trigger_definition TEXT;

ALTER TABLE requirements
    ADD COLUMN IF NOT EXISTS input_definition JSONB;

ALTER TABLE requirements
    ADD COLUMN IF NOT EXISTS output_definition JSONB;
...
ALTER TABLE requirements
    ADD COLUMN IF NOT EXISTS data_requirements JSONB;

ALTER TABLE requirements
    ADD COLUMN IF NOT EXISTS integration_requirements JSONB;

ALTER TABLE requirements
    ADD COLUMN IF NOT EXISTS human_action_required BOOLEAN DEFAULT FALSE;

ALTER TABLE requirements
    ADD COLUMN IF NOT EXISTS constraints TEXT;

ALTER TABLE requirements
    ADD COLUMN IF NOT EXISTS automation_boundary VARCHAR(50) DEFAULT 'UNKNOWN';
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'requirements_analysis_id_fkey'
    ) THEN
        ALTER TABLE requirements
            ADD CONSTRAINT requirements_analysis_id_fkey
            FOREIGN KEY (analysis_id)
            REFERENCES requirements_analyses(id)
            ON DELETE CASCADE;
    END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_reqs_analysis ON requirements(analysis_id);
...
CREATE INDEX IF NOT EXISTS idx_req_analysis_lead ON requirements_analyses(lead_id);
CREATE INDEX IF NOT EXISTS idx_reqs_analysis ON requirements(analysis_id);
CREATE INDEX IF NOT EXISTS idx_reqs_type ON requirements(type);
CREATE INDEX IF NOT EXISTS idx_reqs_priority ON requirements(priority);
CREATE INDEX IF NOT EXISTS idx_reqs_status ON requirements(status);
CREATE INDEX IF NOT EXISTS idx_req_evidence_req ON requirement_evidence(requirement_id);
CREATE INDEX IF NOT EXISTS idx_req_deps_source ON requirement_dependencies(source_req_id);
CREATE INDEX IF NOT EXISTS idx_req_deps_target ON requirement_dependencies(target_req_id);
CREATE INDEX IF NOT EXISTS idx_req_conflicts_a ON requirement_conflicts(req_a_id);
CREATE INDEX IF NOT EXISTS idx_req_conflicts_b ON requirement_conflicts(req_b_id);
CREATE INDEX IF NOT EXISTS idx_req_assumptions_req ON requirement_assumptions(req_id);